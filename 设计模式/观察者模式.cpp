...
class Stock: public Subject{
    private:
        double price =0.0;//股票价格
    public:
        double getPrice();//获取股票价格
        void setPrice();//设置股票价格
...
struct Billboard: Observer{
    ...
    void display(double p) const{
        std::cout<<"Billboard: "<<p <<'\n';
    }
    void update() override{
...
void Stock::attach(Observer* obj){
    observerList.push_back(obj);
}
void Stock::detach(Observer* obj){
